...
<?php
  if (empty($this->input->get('cari')) &&
    ((count((array)$slide_galeri) > 0 || count((array)$slide_artikel) > 0)) && 
      $this->uri->segment(2) != 'kategori') {
    $this->load->view($folder_themes .'/partials/slider');
  }
?>

<?php if(empty($this->input->get('cari')) && $headline && $this->uri->segment(2) != 'kategori') : ?>
  <?php $this->load->view($folder_themes .'/partials/headline') ?>
<?php endif ?>

  <section id="news-list">
    <h3 class="content__heading --mb-4 <?php empty($this->input->get('cari')) && $headline && $this->uri->segment(2) != 'kategori' && print('--mt-5') ?>"><i class="fa fa-newspaper-o"></i> <?= $title ?></h3>
